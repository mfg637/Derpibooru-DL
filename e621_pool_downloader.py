...
try:
    request_url = ("https://e621.net/pools/{}.json?login={}&api_key={}").format(
        pool_id, config.e621_login, config.e621_API_KEY
    )
    headers = {
        'User-Agent': 'e621_pool_downloader.py (by mfg637) (https://github.com/mfg637/Derpibooru-DL)'
    }
    ...
    for album_order, origin_content_id in enumerate(pool_data['post_ids']):
        ...
        set_id = medialib_db.tags_indexer.check_tag_exists(pool_title, 'set', medialib_db_connection)
        if set_id is None:
            set_id = medialib_db.tags_indexer.insert_new_tag(
                pool_title, 'set', "set: {}".format(pool_title), medialib_db_connection
            )

        artist_id = medialib_db.tags_indexer.check_tag_exists(album_artist, 'artist', medialib_db_connection)
        if artist_id is None:
            artist_id = medialib_db.tags_indexer.insert_new_tag(
                album_artist, 'artist', "artist:{}".format(pool_title), medialib_db_connection
            )

        album_id = medialib_db.get_album_id(set_id, artist_id, medialib_db_connection)
        if album_id is None:
            album_id = medialib_db.make_album(set_id, artist_id, medialib_db_connection)
        ...
        if content_info is not None:
            ...
            medialib_db.set_album_order(album_id, content_id, album_order, medialib_db_connection)
...
for origin_content_id in album_order_dict:
    ...
    content_id = content_info[0]
    medialib_db.set_album_order(album_id, content_id, album_order_dict[origin_content_id], medialib_db_connection)
