...
function get_url() {
  ...
  switch (hostname) {
    ...
    default:
      alert(
        `Implementation error: site ${hostname} is not implemented! (line 56)`,
      );
      break;
  }
  return url;
...
function dl_button_click_handler(event) {
  ...
  GM.xmlHttpRequest({
    ...
    onload: function (response) {
      const server_response = JSON.parse(response.responseText);
      if (server_response.status === "OK") {
        const hostname = window.location.hostname;
        if (server_response.origin_domain_name === hostname) {
...
function image_handler(data_wrapper, button_placer, bp_arg = null) {
  let raw_data = data_wrapper.dataset,
    data = {};
...
function dummy_styler(dl_button) {
  return dl_button;
}
...
if (
  ...
  if (image_wrappers.length > 0)
    for (let i in image_wrappers) {
      ...
      data_wrapper =
        image_wrappers[i].getElementsByClassName("image-container")[0];
